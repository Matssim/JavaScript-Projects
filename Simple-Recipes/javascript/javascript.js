...
document.getElementById('contactForm').addEventListener('submit',
    function (event) {
        ...
        if (!firstName || !lastName || !phone || !message) {
            ...
            //If all fields are fields are filled out correctly display thank you message
...
//ADDING JS CHALLENGE
//Automatically closes recipe modal 10 seconds after it has been opened *REMOVE THE FORWARD SLASHES ON BELOW LINE TO ACTIVATE FUNCTION
//const recipeTimout = setTimeout(setRecipeTimeout, 10000);

function setRecipeTimeout() {
    for(var index in modals) {
        if (modals[index].style) {
            modals[index].style.display = "none";
        }
